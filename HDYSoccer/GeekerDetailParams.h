//
//  GeekerDetailParams.h
//  HDYSoccer
//
//  Created by bigyelow on 3/29/14.
//  Copyright (c) 2014 bigyelow. All rights reserved.
//

#ifndef HDYSoccer_GeekerDetailParams_h
#define HDYSoccer_GeekerDetailParams_h

// layout
#define LEFT_MARGIN 10.0f

// header
#define HEADER_HEIGHT 100.0f

#define AVATAR_RECT (CGRectMake(LEFT_MARGIN, 10, 100, 100))
#define AVATAR_BORDER_WIDTH 3.0f

#define NAME_TOP_MARGIN 5.0f
#define NAME_LABEL_FONT ([UIFont fontWithName:@"HelveticaNeue" size:15])
#define NAME_LABEL_FONT_COLOR ([UIColor colorWithRed:62/255.0f green:68/255.0f blue:75/255.0f alpha:1.0f])

// header - short introduction
#define INTER_ROW_SPACE 10.0f
#define SHORT_INTRO_FONT ([UIFont fontWithName:@"HelveticaNeue" size:12])

#define QUALITY_LEFT_MARGIN 10.0f
#define QUALITY_TITLE @"综合素质 - %@"
#define QUALITY_Y 25.0f

#define CHARACTER_TITLE @"特点 - %@"

#define POSITION_TITLE @"位置 - %@"

#endif
